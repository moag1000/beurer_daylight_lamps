...
- Match HA's `async_track_state_added_domain` callback signature in `sensor._make_person_listener` (`Callable[[Event[EventStateChangedData]], Any]`). Resolves mypy error on CI.

## [1.36.2] - 2026-05-17

### Fixed

- Fix CI mypy errors and test_init regression from v1.36.x (`therapy_hub.get_or_create_hub` cast, `sensor._make_person_listener` type annotation, `therapy_hub.ensure_device` tolerates unknown config entries in test mocks).

## [1.36.1] - 2026-05-17

### Fixed

- Fix CI lint errors and JSON syntax issues from v1.36.0 release.
...
- **Event `beurer_daylight_lamps_therapy_session`**: fired on every session end with full payload: `entry_id`, `lamp_name`, `light_entity_id`, `person_id`, `start_time` (ISO), `end_time` (ISO), `duration_minutes`, `color_temp_kelvin`, `brightness_pct`.
...
- **Always-on control**: Lamp stays controllable after disconnect. Commands automatically trigger reconnection, eliminating the need for manual reconnect. Entity remains "available" in HA once the first connection succeeds.
- **Assumed state indicator**: Light entity reports `assumed_state` when not actively connected, so HA UI reflects that the displayed state may be stale.
